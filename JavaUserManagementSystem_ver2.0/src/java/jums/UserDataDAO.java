...
//        
//            st.setString(, "%"+ud.getName()+"%");
//            st.setString(2, "%"+ new SimpleDateFormat("yyyy").format(ud.getBirthday())+"%");
//            st.setInt(3, ud.getType());
            
            ResultSet rs = st.executeQuery();
            //
            ArrayList<UserDataDTO> rchkList = new ArrayList<UserDataDTO>();
            
            while(rs.next()){//行からデータを取得(１つずつ)
            //取り出し対象のレコードの各列の値を取得
            ...
            //whileで取得したデータをchkListに入れる
            rchkList.add(resultUd);
            }
